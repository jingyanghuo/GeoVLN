...
sys.path.append('/home/sq/git2/vln/vlnhub/Matterport3DSimulator/build/')
...
def read_depth_normal(test_only=False):
    import csv
    import base64
    from tqdm import tqdm

    print("Start loading the depth and normal maps... (~50 seconds)")
    start = time.time()
    ...
    if not test_only:
        ...
        dep = {}
        normal = {}
        with open('./img_features/emDepth-CLIP-ResNet-50X4-views.tsv', "r") as tsv_in_file:     # Open the tsv file.
            reader = csv.DictReader(tsv_in_file, delimiter='\t', fieldnames=tsv_fieldnames)
            for item in reader:
                long_id = item['scanId'] + "_" + item['viewpointId']
                dep[long_id] = np.frombuffer(base64.decodestring(item['features'].encode('ascii')),
                                                   dtype=np.float32).reshape((views, -1)).copy()   # Feature of long_id is (36, 2048)
        with open('./img_features/emnormal-CLIP-ResNet-50X4-views.tsv', "r") as tsv_in_file:     # Open the tsv file.
            reader = csv.DictReader(tsv_in_file, delimiter='\t', fieldnames=tsv_fieldnames)
            for item in reader:
                long_id = item['scanId'] + "_" + item['viewpointId']
                normal[long_id] = np.frombuffer(base64.decodestring(item['features'].encode('ascii')),
                                                   dtype=np.float32).reshape((views, -1)).copy()   # Feature of long_id is (36, 2048)
                features[long_id] = np.concatenate((dep[long_id], normal[long_id]), axis=-1)

    else:
        features = None

    print("Finish Loading the depth and normal maps in %0.4f seconds" % (time.time() - start))
    return features
