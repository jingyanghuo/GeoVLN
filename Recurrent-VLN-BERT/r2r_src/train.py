...
from utils import read_vocab, write_vocab, build_vocab, padding_idx, timeSince, read_img_features, print_progress, read_depth_normal
import utils
...
CLIPViT_FEATURES = 'img_features/CLIP-ViT-B-32-views.tsv'
CLIPResNet50x4_FEATURES = 'img_features/CLIP-ResNet-50x4-views.tsv'

if args.features == 'imagenet':
    features = IMAGENET_FEATURES
elif args.features == 'places365':
    features = PLACE365_FEATURES
elif args.features == 'clipvit':
    features = CLIPViT_FEATURES
elif args.features == 'clipresnet50_4':
    features = CLIPResNet50x4_FEATURES
...
def train_val(test_only=False):
    ...
    dep_normal_dict = read_depth_normal(test_only=test_only)
    ...
    train_env = R2RBatch(feat_dict, dep_normal_dict, batch_size=args.batchSize, splits=['train'], tokenizer=tok, obj_store=obj_store,
    ...
    val_envs = OrderedDict(
        ((split,
          (R2RBatch(feat_dict, dep_normal_dict, batch_size=args.batchSize, splits=[split], tokenizer=tok, obj_store=obj_store, mp_feature_store=mp_feat_dict),
...
def train_val_augment(test_only=False):
    ...
    train_env = R2RBatch(feat_dict, dep_normal_dict, batch_size=args.batchSize, splits=['train'], tokenizer=tok_bert, obj_store=obj_store,
                         mp_feature_store=mp_feat_dict)
    aug_env   = R2RBatch(feat_dict, dep_normal_dict, batch_size=args.batchSize, splits=[aug_path], tokenizer=tok_bert, name='aug', obj_store=obj_store,
                         mp_feature_store=mp_feat_dict)

    # Setup the validation data
    val_envs = {split: (R2RBatch(feat_dict, dep_normal_dict, batch_size=args.batchSize, splits=[split], tokenizer=tok_bert, obj_store=obj_store,
                                 mp_feature_store=mp_feat_dict),
