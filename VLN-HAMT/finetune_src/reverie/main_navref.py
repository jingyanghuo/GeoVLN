...
from reverie.model_navref import get_tokenizer
from reverie.agent import NavRefCMTAgent
from reverie.data_utils import ImageFeaturesDB, construct_instrs
...
def build_dataset(args, rank=0, is_test=False):
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db, obj_db, val_instr_data, args.connectivity_dir, args.anno_dir, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, REF_loss %.4f, RL_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, REF_loss, RL_loss, policy_loss, critic_loss),
                record_file
            )
        ...
        for env_name, env in val_envs.items():
            ...
            if default_gpu:
                ...
                if env_name in best_val:
                    if score_summary['spl'] + score_summary['rgspl'] >= best_val[env_name]['spl'] + best_val[env_name]['rgspl']:
