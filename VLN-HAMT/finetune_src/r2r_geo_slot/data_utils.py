...
class ImageFeaturesDB(object):
    ...
    def get_image_feature(self, scan, viewpoint):
        ...
        if key in self._feature_store:
            ...
            with h5py.File('../datasets/R2R/features/dep_feature.hdf5', 'r') as f:
                ft_dep = f[key][...][:, :self.image_feat_size].astype(np.float32)
            with h5py.File('../datasets/R2R/features/normal_feature.hdf5', 'r') as f:
                ft_normal = f[key][...][:, :self.image_feat_size].astype(np.float32)
                
            ft = np.concatenate((ft, ft_dep, ft_normal), -1)
