...
from utils.misc import length2mask, localmask
...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def _cand_pano_feature_variable(self, obs):
        ...
        pano_img_fts = np.zeros((len(obs), 36, 768), np.float32)
        ...
        for i, ob in enumerate(obs):
            ...
            ob_img_fts.append(cand_img_fts)
            ob_ang_fts.append(cand_ang_fts)
            ob_nav_types.append(cand_nav_types)

            # pano image 36
            pano_fts = ob['feature']
            pano_img_fts[i] = pano_fts[:, :self.args.image_feat_size]
            pano_ang_fts[i] = pano_fts[:, self.args.image_feat_size:]

        # pad features to max_len
        max_len = max(ob_cand_lens)
        for i in range(len(obs)):
            num_pads = max_len - ob_cand_lens[i]
            ...
            ob_nav_types[i] = np.array(ob_nav_types[i] + [0] * (max(ob_lens) - ob_lens[i]))
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.args.slot_attn:
                ...
                candidate_mask = length2mask(candidate_leng)
                slot_candidate_mask = candidate_mask

                if self.args.slot_local_mask:
                    pointIds = [
                        [cand['pointId'] for cand in ob['candidate']] for ob in obs
                    ]
                    local_mask = localmask(pointIds, max(candidate_leng), 36)
                ...
                slot_result, slot_attn_weight = self.slot_attention(torch.cat([ob_img_feats, ob_ang_feats], -1), 
                        torch.cat([pano_img_feats, pano_ang_feats], -1), slot_candidate_mask)
                if self.args.slot_residual:
                    ob_img_feats = ob_img_feats + slot_result[..., : -self.args.angle_feat_size]
                else:
                    ob_img_feats = slot_result[..., : -self.args.angle_feat_size]
            
            # pad features to max_len
            num_pads = max(ob_lens)-max(ob_cand_lens)
            ob_img_feats = torch.cat([ob_img_feats, torch.zeros(self.args.batch_size, num_pads, ob_img_feats.shape[-1]).cuda()], 1)
            ob_ang_feats = torch.cat([ob_ang_feats, torch.zeros(self.args.batch_size, num_pads, ob_ang_feats.shape[-1]).cuda()], 1)
            ...
            for i, ob in enumerate(obs):
                ...
                # add pano context
                pano_fts = ob['feature'][~cand_pointids]
                ob_img_feats[i, ob_cand_lens[i]:ob_cand_lens[i]+sum(~cand_pointids)] = pano_img_feats[i, ~cand_pointids, :].clone()
                ob_ang_feats[i, ob_cand_lens[i]:ob_cand_lens[i]+sum(~cand_pointids)] = pano_ang_feats[i, ~cand_pointids, :].clone()
