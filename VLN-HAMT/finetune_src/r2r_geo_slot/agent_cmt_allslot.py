...
from utils.misc import length2mask, localmask_allslot
...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        if self.args.slot_attn:
            self.slot_attention = SlotAttention(
                num_slots=1,
                dim=(768 + 4),
                drop_rate=args.slot_dropout,
            ).cuda()
            self.slot_optimizer = optimizer(self.slot_attention.parameters(), lr=args.lr)
            self.models.append(self.slot_attention)
            self.optimizers.append(self.slot_optimizer)

        if self.args.lr_adjust_type == 'cosine':
            self.schedulers = [
    ...
    def _cand_pano_feature_variable(self, obs):
        ...
        pano_img_fts = np.zeros((len(obs), 36, self.args.image_feat_size*3), np.float32)
        pano_ang_fts = np.zeros((len(obs), 36, 4), np.float32)
        for i, ob in enumerate(obs):
            cand_img_fts, cand_ang_fts, cand_nav_types = [], [], []
            cand_pointids = np.zeros((self.args.views, ), dtype=np.bool)
            for j, cc in enumerate(ob['candidate']):
                cand_img_fts.append(cc['feature'][:self.args.image_feat_size*3])
                cand_ang_fts.append(cc['feature'][self.args.image_feat_size*3:])
                cand_pointids[cc['pointId']] = True
                cand_nav_types.append(1)
            # add [STOP] feature
            cand_img_fts.append(np.zeros((self.args.image_feat_size*3, ), dtype=np.float32))
            ...
            cand_pano_img_fts = np.concatenate([cand_img_fts, pano_fts[:, :self.args.image_feat_size*3]], 0)
            cand_pano_ang_fts = np.concatenate([cand_ang_fts, pano_fts[:, self.args.image_feat_size*3:]], 0)
            ...
            # pano image 36
            pano_img_fts[i] = ob['feature'][:, :self.args.image_feat_size*3]
            pano_ang_fts[i] = ob['feature'][:, self.args.image_feat_size*3:]
        ...
        pano_img_fts = torch.from_numpy(pano_img_fts).cuda()
        pano_ang_fts = torch.from_numpy(pano_ang_fts).cuda()

        return ob_img_fts, ob_ang_fts, ob_nav_types, ob_lens, ob_cand_lens, pano_img_fts, pano_ang_fts
    ...
    def _history_variable(self, obs):
        if self.args.history_fusion:
            img_feat_size = self.args.image_feat_size*3
        else:
            img_feat_size = self.args.image_feat_size
        hist_img_feats = np.zeros((len(obs), img_feat_size), np.float32)
        for i, ob in enumerate(obs):  
            hist_img_feats[i] = ob['feature'][ob['viewIndex'], :img_feat_size]
        hist_img_feats = torch.from_numpy(hist_img_feats).cuda()

        if self.args.hist_enc_pano:
            hist_pano_img_feats = np.zeros((len(obs), self.args.views, img_feat_size), np.float32)
            hist_pano_ang_feats = np.zeros((len(obs), self.args.views, self.args.angle_feat_size), np.float32)
            for i, ob in enumerate(obs):
                hist_pano_img_feats[i] = ob['feature'][:, :img_feat_size]
                hist_pano_ang_feats[i] = ob['feature'][:, self.args.image_feat_size*3:]
            ...
            hist_pano_img_feats, hist_pano_ang_feats = None, Non
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.args.max_action_len):
            if self.args.ob_type == 'pano':
                ob_img_feats, ob_ang_feats, ob_nav_types, ob_lens, ob_cand_lens, pano_img_feats, pano_ang_feats = self._cand_pano_feature_variable(obs)
            ...
            if self.args.slot_attn:
                slot_candidate_mask = length2mask(ob_lens)
                # slot_candidate_mask = ob_masks
                if self.args.slot_local_mask:
                    pointIds = []
                    for ob in obs:
                        p_id = [cand['pointId'] for cand in ob['candidate']]
                        p_one = -np.ones(1, dtype=int)
                        full_p_id = np.concatenate([p_id, p_one, [idx for idx in list(range(36)) if idx not in p_id]], 0)
                        pointIds.append(full_p_id)
                    
                    local_mask = localmask_allslot(pointIds, max(ob_lens), 36)
                ...
                slot_result, slot_attn_weight = self.slot_attention(torch.cat([ob_img_feats[..., :self.args.image_feat_size], ob_ang_feats], -1), 
                        torch.cat([pano_img_feats[..., :self.args.image_feat_size], pano_ang_feats], -1), slot_candidate_mask)
                if self.args.slot_residual:
                    ob_img_feats[..., :self.args.image_feat_size] = ob_img_feats[..., :self.args.image_feat_size] + slot_result[..., : -self.args.angle_feat_size]
                else:
                    ob_img_feats[..., :self.args.image_feat_size] = slot_result[..., : -self.args.angle_feat_size]
